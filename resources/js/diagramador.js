...
function init() {
    ...
    miDiagrama.addDiagramListener("Modified", e => {
        console.log('acabo de ejecutar el Modified()');
        const button = document.getElementById("btn_guardar");

        button.addEventListener('click', function () {
            console.log('le di click en guardar');
            //buscar el nodo seleccionado y actualizar ubicacion en la base de datos
            miDiagrama.isModified;

        });


            // console.log('cambios2: ', go.ChangedEvent.Property);
            // if (e.change === go.ChangedEvent.Property) {
            //     // console.log("La propiedad " + e.propertyName + " cambió.");
            //     console.log("La propiedad " + e.property + " cambió.");
            // }
        });
            ...
            $GG(go.Group, "Vertical",
                {
                    ...
                    click: function (e, node) {
                        ...
                        miNodo = miDiagrama.model.findNodeDataForKey(node.data.key);
                        console.log('Mi Nodo: ', miNodo);
                        key_artefacto = miNodo.key;
                        loc = go.Point.parse(miNodo.loc);
                        loc.x += 100;
                        console.log('loc del nodo: ', loc);
                    },
                },
    ...
    var grupo = [];
    var enlace = [];
    console.log('artefactos: ', artObjetos);
    ...
    function guardarArtefacto(newNodeData) {
        ...
        formulario.append("loc", newNodeData.loc);
